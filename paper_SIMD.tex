...
We want to build a graph $G'$ with fewest possible vertices since fewer vertices means more parallelization. \ishaq{TODO: make it more concrete, pick some variables e.g. $\alpha, \beta$ like above and show 1) parallel schedule will have lower cost (this should be easy). and 2) some sort of optimality? this would be hard}

\subsection{Scheduling is NP-hard}
\label{sec:np}

\ana{Ishaq?}\ishaq{TODO: As a first step, pull the super-sequence proof from Lindsay's thesis? }
...
We write $A[i,j,k]$ to denote canonical access to an array element. \ana{Need to state this more precisely.}
...
\ana{TOTO Ana: Shorten this section. Lots of this is unnecessary.}
...
\begin{itemize}
\item same-level forward $X\rightarrow Y$ where $X$ and $Y$ are in the same loop nest $i_1,i_2,...i_k$. E.g., $\texttt{d} = \mathit{SUB}(\texttt{S}[i,j], \texttt{C}[j])$ to $\texttt{p} = \mathit{MUL}(\texttt{d},\texttt{d})$ in Biometric is a same-level edge. A $\phi$ node can be a source of a same-level forward edge but not a target.
...
\ana{TODO: Still work in progress, all over.}
...
\COMMENT { Phase 3: Dimensionality reduction: removing unnecessary dimensionality. }
...
\subsection{Extension of Basic Vectorization}
...
First, we merge closures that update the same array. \ana{Why, intuition?} Second, we add the target-less node of array \texttt{A} back to the closure that updates \texttt{A} ---
...
The canonical dimensionality of \texttt{res} is 1. Also, the phi-node \texttt{res2 = PHI(res1, res3)} is a target-less phi-node (the implication being that the inner for loop can be vectorized across $i$). After Phase 1, Basic vectorization produces the following (everything is implicitly vectorized along $i$ and $j$ and the update is turned into a regular write):
...
for i in range(0, num_bins):   res1 = PHI(res, res2) // target-less, no drop_dim
   res1' = raise_dim(res1, (j:N))   for j in range(0, N):      res2 = PHI(res1', res3)      tmp1 = (A1 == I)      tmp2 = (res2 + B1)      tmp3 = MUX(tmp1, res2, tmp2)
      res3 = res2 // copy propagation: res2 into res3      res3 = tmp3 // "normal": res3[I,J] = tmp3[I,J]    return res1
\end{verbatim}
...
res1' = raise_dim(res, (j:N)) // raising original array to a 2-dimensional arrayfor j in range(0, N):   res2 = PHI(res1', res3)   tmp2[I,j] = (res2[I,j] + B1[I,j])   tmp3[I,j] = MUX(tmp1[I,j], res2[I,j], tmp2[I,j])   res3 = res2
   res3[I,j] = tmp3[I,j]
res1 = drop_dim(res2) // Have to add drop_dim before removing redundant phi-node      return res1
\end{verbatim}

The cleanup phase can then get rid of $j$ in the res variables and tmp variables, so the loop operates over the num\_bins-size vectors. 
...
for j in range(0, N):   res2 = PHI(res1', res3) // now 1-dim array   tmp2[I] = (res2[I] + B1[I,j])   tmp3[I] = MUX(tmp1[I,j], res2[I], tmp2[I])   res3 = res2
   res3[I] = tmp3[I]return res2
\end{verbatim}
