...
\item inner-to-outer back-edge $X\rightarrow Y$: $X$ and $Y$ are both $\phi$-nodes for some variable. The source $X$ is in a loop nest immediately nested into $Y$'s loop nest. 
 \end{itemize}
...
\ishaq{Naive question: Which index of $\texttt{min}_{1}[\stackrel{\rightarrow}{i}]$ would contain the minimum, how are we reducing it?}
...
\ishaq{For our rader: Such dependencies will not involve phi nodes only. e.g. $a[i] = a[i] + a[i-1]$}
...
\ishaq{I don't think I understand closures. Why does the closure for $\texttt{index}_0[i]$ include $\texttt{c}[\stackrel{\rightarrow}{i}]$ but the closure for $\texttt{index}_0[i] = \phi(\texttt{index}_1[i-1], -1)$ not include $\texttt{p}$.}
...
references the \emph{same} array element as an use $ ... = \texttt{A}_n[f'(i,j,k)]$ at some later iteration $\underline{j}'$, 
...
Next, the algorithm refines array MUX statement. 
...
index or a pair of indices, e.g., $\texttt{A}_j[i] = \emph{MUX}(c, \texttt{A}_k[i], \texttt{A}_l[i])$. \ana{Clarify why this helps. Still work in progress.}
...
\IF {$i_1 \cong i_2$} \STATE \COMMENT{ Also $i_1 \cong i_2$ when one is null. WLG assume $i_2$ is null } 
\STATE replace $\mathit{stmt}$ with \\
\STATE $\texttt{A}_j = \texttt{A}_k$; $\texttt{A}_j[i_1] = \emph{MUX}(c, \texttt{A}_k[i_1], \texttt{A}_l[i_1])$
%\ELSIF {$i_1 \ncong i_2$}  
%\STATE replace $\mathit{stmt}$ with \\
%\STATE $\texttt{A}_j = \texttt{A}_k$; $\texttt{A}_j[i_1] = \emph{MUX}(c, \texttt{A}_k[i_1], \texttt{A}_l[i_1])$
\ELSE \STATE \COMMENT{ Also, not $\cong$ when one is $\texttt{A}_j = \emph{MUX}(..., \texttt{A}_k, \texttt{A}_l)$}
