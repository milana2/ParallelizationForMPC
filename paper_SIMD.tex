...
Recall that after removal of infeasible edges, the Aiken's array write example will be (roughly) as follows:
...
\STATE 5. $\texttt{A}_2$ = update($\texttt{A}_1$, i, $\texttt{B}_1$[i] + 10);
\STATE 6. $\texttt{B}_2$ = update($\texttt{B}_1$, i, $\texttt{A}_2$[i] * $\texttt{D}_1$[i-1]);
\STATE 7. $\texttt{C}_2$ = update($\texttt{C}_1$, i, $\texttt{A}_2$[i] * $\texttt{D}_1$[i-1]);
\STATE 8. $\texttt{D}_2$ = update($\texttt{D}_1$; i, $\texttt{B}_2$[i] * $\texttt{C}_2$[i]);
\end{algorithmic}

There are no nested loops, thus, array accesses remain as is. The closure of 4 includes 4,6,7, and 8, 
while 5 is vectorizable. 


Removal of infeasible edges leaves $\phi$-nodes 1,2, and 3, i.e., A,B, and C as targetless. 
