...
\COMMENT {Phase 4: Dimensionality reduction: remove unnecessary dimensionality. \ana{Better draft! Needs a bit more work.}}

\COMMENT { A dimension i is dead on exit from stmt v[...i...] = ... if all def-uses with targets outside of the enclosing 
FOR i=0 ... MOTION loop end at target (use) v' = drop\_dim(v, i). } 
\FOR {each stmt and dimension v[...i...] = ...}
\STATE {if i is a dead dimension on exit from stmt v[...i...] = ..., remove i from v[...i...] (at def and at all uses)} 
...
\FOR {each v' =  drop\_dim(v,i) }
\STATE {replace with v' = v if i is dead in v.}
\ENDFOR
\FOR {each use v'[...i...] in loop with def in outer scope}
\STATE {drop to lowest dimension}
\ENDFOR
