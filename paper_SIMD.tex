...
As a running example, we assume a standard MPC benchmark, Biometric matching.
...
Array \texttt{C} is the feature vector of \texttt{D} 
features that we wish to match and array \texttt{S} is the database of \texttt{N} vectors of size \texttt{D}
that we match against. The programmer annotates the inputs and outputs as \emph{shared}.
The program iterates over the entries in the database and computes
...
Our compiler imposes the following restrictions. We note that in most cases, the restrictions 
...
We linearize the source turning conditionals into MUX statements. The PHI nodes
are remnants of the SSA IR; our compiler generates code that picks the correct value 
when producing MOTION output and the MOTION framework in turn linearizes 
MPC source loops when it generates the circuit.
...
Our key contribution is the vectorizing transformation. We can compute all $N*D$ 
...
Our compiler automatically detects these opportunities and transforms the program. 
...
\subsection{Vectorization and Cost of Schedule}

Our compiler produces the following vectorized program. (Note that this is
still higher-level IR, our compiler turns this code into MOTION variables, 
