...
\begin{enumerate}
\item All statements in the program execute using the same protocol (sharing). That is, there is no share conversion. \ishaq{We should say somewhere (in a footnote?) that we know this is not ideal, otherwise reviewers will complain.}
\item All MPC instructions have the same unit cost, 1 unit. \ishaq{Don't we need to distinguish between local (cheap) vs. interactive (expensive) instructions? I don't see how we could assume this. -- Past suggestion from Vassilis: Ideally we should assume cost to be a convex function.}
\item There is unlimited bandwidth---i.e., a single MPC-instruction costs as much as $N$ amortized instructions, namely 1 unit.\ishaq{This is fair for theoretical model, although in some real life applications this is not true, e.g. L2 norm of large vectors.}
...
in parallel still cost 2. \ishaq{This is not true, can we still assume it? -- YES.} 
\end{enumerate}
...
\ana{TODO: We need to remove phi-nodes that end up without incoming back-edges.} 
...
\STATE for i in range(N):
\STATE 1. $\texttt{D}_0$ = $\phi(\texttt{D},\texttt{D}_1)$ 
\STATE 2. $\texttt{A}_1$ = $\texttt{A}$; $\texttt{A}_1[\stackrel{\rightarrow}{i}] = \texttt{B}[\stackrel{\rightarrow}i] + [10,...]$
\STATE 3. $\texttt{B}_1$ = $\texttt{B}$; $\texttt{B}_1[\stackrel{\rightarrow}i] = \texttt{A}_1[\stackrel{\rightarrow}i] * \texttt{D}_0[\stackrel{\rightarrow}{i-1}]$
\STATE 4. $\texttt{C}_1$ = $\texttt{C}$; $\texttt{C}_1[\stackrel{\rightarrow}i] = \texttt{A}_1[\stackrel{\rightarrow}i] * \texttt{D}_0[\stackrel{\rightarrow}{i-1}]$
\STATE 5. $\texttt{D}_1$ = $\texttt{D}_0$; $\texttt{D}_1[\stackrel{\rightarrow}i] = \texttt{B}_1[\stackrel{\rightarrow}i] * \texttt{C}_1[\stackrel{\rightarrow}i]$
\end{algorithmic}
...
\begin{algorithmic}
\STATE 1. $\texttt{A}_1$ = $\texttt{A}$; $\texttt{A}_1[\stackrel{\rightarrow}{i}] = \mathit{ADD\_SIMD}(\texttt{B}[\stackrel{\rightarrow}i],[10,...])$ \COMMENT{Fully vectorized, size N.} 
\STATE FOR i=0; i<N; i++; \COMMENT{ MOTION loop }
\STATE 2. $\texttt{D}_0$ = $\phi(\texttt{D},\texttt{D}_1)$ 
\STATE 3. $\texttt{B}_1$ = $\texttt{B}$; $\texttt{B}_1[i] = \mathit{MUL}(\texttt{A}_1[i], \texttt{D}_0[{i-1}])$
\STATE 4. $\texttt{C}_1$ = $\texttt{C}$; $\texttt{C}_1[i] = \mathit{MUL}(\texttt{A}_1[i], \texttt{D}_0[{i-1}])$
\STATE 5. $\texttt{D}_1$ = $\texttt{D}_0$; $\texttt{D}_1[i] = \mathit{MUL}(\texttt{B}_1[i], \texttt{C}_1[i])$
\end{algorithmic}
