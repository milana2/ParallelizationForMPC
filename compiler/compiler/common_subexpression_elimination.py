...
def _check_and_modify_rhs(
    ...
    if not isinstance(stmt, Return) and (hasattr(stmt, 'rhs') and str(stmt.rhs) in dag):
        if hasattr(stmt.lhs, 'array') and isinstance(stmt.rhs, DropDim) and isinstance(stmt.rhs.array, VectorizedAccess):
            changes[str(stmt.lhs.array)] = dag[str(stmt.rhs)].array
        if hasattr(stmt.lhs, 'array') and isinstance(stmt.rhs, LiftExpr):
            changes[str(stmt.lhs.array)] = dag[str(stmt.rhs)].array
        if hasattr(stmt.lhs, 'array') and isinstance(stmt.rhs, BinOp) and isinstance(stmt.lhs, VectorizedAccess):
            changes[str(stmt.lhs.array)] = dag[str(stmt.rhs)].array
        if hasattr(stmt.lhs, 'array') and isinstance(stmt.rhs, Tuple) and isinstance(stmt.lhs, VectorizedAccess):
            changes[str(stmt.lhs.array)] = dag[str(stmt.rhs)].array
        if hasattr(stmt.lhs, 'array') and isinstance(stmt.rhs, Mux) and isinstance(stmt.lhs, VectorizedAccess):
            changes[str(stmt.lhs.array)] = dag[str(stmt.rhs)].array
        if hasattr(stmt.lhs, 'array') and isinstance(stmt.rhs, VectorizedUpdate):
            changes[str(stmt.lhs.array)] = dag[str(stmt.rhs)].array
        if hasattr(stmt.lhs, 'array') and isinstance(stmt.rhs, UnaryOp) and isinstance(stmt.lhs, VectorizedAccess):
            changes[str(stmt.lhs.array)] = dag[str(stmt.rhs)].array
        if hasattr(stmt.lhs, 'array') and (isinstance(stmt.rhs, Subscript) or isinstance(stmt.rhs, Update)):
            changes[str(stmt.lhs.array)] = dag[str(stmt.rhs)].array
        ...
        if isinstance(stmt.lhs, VectorizedAccess) and hasattr(dag[stmt_splits[1]], 'array'):
            ...
            if changes[stmt_splits[0]] is None:
        ...
        if isinstance(stmt, For):
            ...
            if stmt.value in dag:
                return Return(dag[str(stmt.value)])
            return stmt
        elif isinstance(stmt.rhs, Constant):
            return stmt
        elif isinstance(stmt.rhs, LiftExpr):
            if isinstance(stmt.rhs, VectorizedAccess) and hasattr(stmt.rhs.expr, 'array') and str(stmt.rhs.expr.array) in changes:
                if isinstance(stmt.rhs.expr, Subscript):
                    stmt.rhs.expr = Subscript(changes[str(stmt.rhs.expr.array)], stmt.rhs.expr.index)
                else:
            ...
            if isinstance(stmt.lhs, VectorizedAccess) and hasattr(stmt.rhs.operand, 'array') and str(stmt.rhs.operand.array) in changes:
                if isinstance(stmt.rhs.operand, Subscript):
                    stmt.rhs.operand = Subscript(changes[str(stmt.rhs.operand.array)], stmt.rhs.operand.index)
                else:
            ...
            if str(stmt.rhs.array) in changes:
                if isinstance(stmt.rhs, Subscript):
                    stmt.rhs = Subscript(changes[str(stmt.rhs.array)], stmt.rhs.index)
                else:
                    stmt.rhs.array = changes[str(stmt.rhs.array)]
            if str(stmt.rhs) in dag:
                dag[str(stmt.lhs)] = dag[str(stmt.rhs)]
