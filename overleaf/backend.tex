...
\begin{enumerate}
\item Loop counters and array bounds are always \texttt{plain}.
...
The final rule is needed in examples like the following. In the below snippet {\sf sum!2} and {\sf sum!3} form a dependency cycle and there is no \texttt{shared} value that flows to either one.
...
\ben{The note in parentheses is not true: we currently assume that all plaintext variables other than loop counters will be used in a shared expression.  We initialize a shared version of them in the preamble (using the \texttt{\_MPC\_CONSTANT} variables or the plaintext parameter, depending on how the plaintext variable is initialized) and update that shared variable whenever the plaintext variable is updated.  This is done to minimize the number of input gates (e.g. if a plaintext variable is upcast to shared in a loop, that would insert N input gates).  With protocols that allow publicly known shared constants, this would not be necessary.  This is described in the final subsection of this section.}\ana{Got it, thanks! I think it is good now.}
...
Similarly, the {\sf vectorized\_assign()} function assigns a (potentially SIMD) value to a slice of a vectorized array.  This operation cannot be done with a simple subscript as SIMD assignments will update a range of values in the underlying array representation. \ana{Changed non-vectorized -> array representation. Check if correct.}
