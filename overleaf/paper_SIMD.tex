...
$\mathit{raise\_dim}$ and $\mathit{drop\_dim}$ in the same way. However, the implementation of raise dimension and drop dimension will be adapted because the dimension cannot be raised or drop to a dimension lower than the canonical one. Consider a def-use edge $X \rightarrow Y$ for an array $\texttt{A}$.

\begin{enumerate}

\item same-level $X \rightarrow Y$. We propagate the array, which happens to be of the right dimension. 
...
\item inner-to-outer $X \rightarrow Y$ triggers the addition of $\mathit{drop\_dim}$. However, if the dimensionality of the loop enclosure of $X$ is equal or less than the canonical dimensionality of the array, then $\mathit{drop\_dim}$ has no effect, otherwise its behavior is as in Basic Vectorization.
