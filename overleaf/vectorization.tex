...
for vectorization in intermediate statements in the loop. Second, it handles write arrays. 
...
with array writes. We advise the reader to omit the extension and assume just read-only
arrays. We explain the extension in \secref{sec:array_writes} details our algorithm. 
(As many of our benchmarks include array writes, it plays an important role.)
...
\STATE {create \python{for} $i_d$ \python{in} ... loop}
