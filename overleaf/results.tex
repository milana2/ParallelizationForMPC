...
We run all experiments 5 times and report average values of various metrics. Note that standard deviation in all observations was at most 4.5\% of the mean value. %,  therefore the accuracy of the results is not effected by the relatively fewer runs.

\subsection{Benchmarks}\label{sec:benchmarks_description}

We used benchmarks from ABY and OPA~\cite{Demmler:2015,Ishaq:2019} (Biometric Matching, Histogram, PSI), HyCC~\cite{Buscher:2018b} (Database Join, Database Variance, Inner Product, Cryptonets Max Pooling, MNIST ReLU, k-means Iteration) and Parsynt~\cite{Farzan:2021} (Convex Hull, Minimal Points, Count 102, Count 10, Longest 102, Max distance between symbols). 
We did not use Gcd~\cite{Ishaq:2019} and Modular Exponentiation from~\cite{Demmler:2015} as they do not benefit from vectorization according to our model. 
We ran as many routines from~\cite{Buscher:2018} and~\cite{Farzan:2021} as possible. Gaussian decomposition cannot be expressed 
...
To compare we run {\em both} versions with the maximal value of $N$ for which the non-vectorized version succeeds (typically {\tt N=128}). We also run {\em vec} with larger values, 
typically {\tt N=4096}. Again, due to space constraints \cite{Anon_TR} gives the exact values.
...
% however, they times are largely consistent, e.g., if {\em both} is $N = 2^k$ and it completes in $X$ seconds for vectorized, then $N = 2^{12}$ completes in $(12-k)X$ seconds).
...
\resizebox{0.7\textwidth}{!}{\input{graphs/lan/all-hist-OnlineSetupTimesec}}
...
\begin{figure}[htbp]
\centering
\resizebox{0.7\columnwidth}{!}{\input{graphs/lan/biometric-hist-OnlineSetupTimesec}}
...
\begin{figure}[htbp]
\centering
\resizebox{0.7\columnwidth}{!}{\input{graphs/lan/biometric-hist-CommunicationMiB}}
...
\begin{figure}[htbp]
\centering
\resizebox{0.7\columnwidth}{!}{\input{graphs/lan/biometric-hist-CircuitGenerationTimesec}}
...
\begin{figure}[htbp]
\centering
\resizebox{0.7\columnwidth}{!}{\input{graphs/comparison-hist-OnlineSetupTimesec}}
...
\begin{figure*}[htbp]
\centering
\resizebox{0.7\textwidth}{!}{\input{graphs/lan/all-hist-CommunicationMiB}}
...
\begin{figure*}[htbp]
\centering
\resizebox{0.7\textwidth}{!}{\input{graphs/lan/all-hist-CircuitGenerationTimesec}}
...
\begin{figure*}[htbp]
\centering
\resizebox{0.7\textwidth}{!}{\input{graphs/lan/all-hist-TotalGates}}
...
\begin{figure*}[htbp]
\centering
\resizebox{0.7\textwidth}{!}{\input{graphs/lan/all-hist-OnlineTimesec}}
...
\begin{figure*}[htbp]
\centering
\resizebox{0.7\textwidth}{!}{\input{graphs/lan/all-hist-SetupTimesec}}
...
limitations of our implementation. Recall that Phase 3 of Vectorization in~\secref{sec:vectorization}, which we have not implemented yet, gets rid of redundant 
...
will detect the associative accumulation in the scalar expansion code, one might expect that it will in the above simpler code. 
However, because AST analysis is difficult, it does not appear to detect the accumulation.
%(unfortunately, we ran out of time and did not experiment further).
%Interestingly, the non-vectorized version, which is the literal translation of inner product does not appear to benefit from any optimizations. 
...
HyCC~\cite{Buscher:2018b} is a mixing compiler that focuses on mixed protocols, while we run vectorization within a single protocol. 
The paper only provides data for Boolean and Yao for a version of Biometric matching on $N=1000$ (as its focus is mixing). %; it does provide a lot of data on mixed protocol executions.
