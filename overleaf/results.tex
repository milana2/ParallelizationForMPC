...
{\sf result += mult\_unsimdified[i];} while our loop generated and ran {\sf result[i] = result[i-1] + mult\_unsimdified[i];} (recall that the array computation
...
Therefore, MOTION's compiler performs analysis that informs circuit generation and the example illustrates the power of the analysis. In the above example, 
MOTION does the standard divide-and-conquer accumulation. The poor performance of our loop is not a limitation of our approach, but of our current 
implementation. Recall that the next phase of Vectorization, in~\secref{sec:vectorization}, which we have not implemented yet, gets rid of redundant 
dimensions and generates {\sf result = result\_prev + mult\_unsimdified[i]}. This could have benefited from MOTION's divide-and-conquer optimization
but it might lead to confusion of the static analysis as analysis on the AST is difficult (unfortunately, we ran out of time and did not experiment further).
...
We conjecture that MPC source, as a more straight-forward representation will not only enable detection of general associative loops, but also allow 
